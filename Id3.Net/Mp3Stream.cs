...
namespace Id3
{
    ...
    public class Mp3Stream : IDisposable
    {
        ...
        {
            var familyHandlers = ExistingHandlers.GetHandlers(family);

            var familyHandler = familyHandlers.FirstOrDefault();
            if (familyHandler == null)
                return null;
            var handler = familyHandler.Handler;
            var tag = handler.ReadTag(_stream);
            return tag;
        }
        ...
        public bool WriteTag(Id3Tag tag, WriteConflictAction conflictAction = WriteConflictAction.NoAction)
        {
            ...
            if (tag.MajorVersion == 0)
                throw new ArgumentException(Id3Messages.MajorTagVersionMissing, nameof(tag));
        ...
        public byte[] GetAudioStream()
        {
            ...
            long audioStreamLength = _stream.Length - (startBytes?.Length ?? 0) -
                (endBytes?.Length ?? 0);
        ...
        internal static RegisteredId3Handlers RegisteredHandlers => _registeredHandlers;
